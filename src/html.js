...
export function renderHtml(analysis, { live = false } = {}) {
  if (analysis.empty) {
    return page(
      ...
      '<p class="empty">No snapshots yet — the first poll is on its way.</p>',
      { live },
    );
  }
...
const demoBanner = () => `
  <div class="banner" role="status">
    <strong>Synthetic demo data.</strong> These numbers describe a made-up storm
    generated by <code>node comed.js demo</code>, not a real ComEd outage. Clear
    the <code>data/</code> directory and run <code>node comed.js check</code> for
    live figures.
  </div>`;
...
.banner {
  ...
  padding: 12px 16px; margin-bottom: 24px; font-size: 13px; line-height: 1.5;
}
.live-dot {
  display: inline-block; width: 7px; height: 7px; border-radius: 50%;
  background: var(--restored); margin-right: 6px; vertical-align: middle;
}
</style>
</head>
<body>
...
// Served-mode only: watch for a newer snapshot and reload when one lands, so a
// tab left open keeps pace with the background polling loop.
